...
export function getRoom(roomId: string) {
  ...
  const now = Date.now();
  roomSignals[roomId].signals = roomSignals[roomId].signals.filter((s) => now - s.timestamp < 30000);
  for (const [pid, p] of Object.entries(roomSignals[roomId].participants)) {
    if (now - p.lastSeen > 20000) {
      delete roomSignals[roomId].participants[pid];
    }
  }
...
export async function POST(
  ...
  try {
    ...
    const { senderId, name, role, type, to, data, isCameraOn, isMicOn } = body;
