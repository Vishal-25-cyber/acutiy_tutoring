      ...
      {isCreateModal && (
        <Modal isOpen={isCreateModal} onClose={() => setIsCreateModal(false)} title={`Create ${activeCategory === "TEST" ? "Timed Proctored Test" : activeCategory === "HOMEWORK" ? "Daily Homework" : "Assignment"}`}>
          <form onSubmit={handleCreate} className="space-y-4 pt-2">
            ...
            {/* Test Specific Fields vs Assignment Due Date & Due Time */}
            {activeCategory === "TEST" ? (
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">Test Date *</label>
                    <input
                      type="date"
                      required
                  ...
                  <div className="space-y-1">
                    <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">Test Start Time *</label>
                    <input
                      type="time"
                      required
                      value={formData.dueTime}
                      onChange={(e) => setFormData({ ...formData, dueTime: e.target.value })}
                      className="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 font-mono"
                    />
                  </div>
                </div>
            ...
            <div className="flex gap-2 pt-2">
              <button
                ...
                className="px-4 py-2 text-xs font-bold rounded-xl border border-slate-200 dark:border-slate-700 text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
              >
                Cancel
              </button>
              <button
                type="submit"
                className="flex-1 py-2 text-xs font-bold rounded-xl bg-[#004b79] text-white hover:bg-[#003b60] transition-colors cursor-pointer"
              >
