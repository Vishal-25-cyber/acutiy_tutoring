...
export default function AdminStudentsPage() {
  ...
  const handleQuickApprove = async (st: any, approvePayment = true) => {
    try {
      const res = await fetch("/api/admin/students", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const totalPendingApprovals = students.filter(
    (st: any) =>
      st.userId?.status === "PENDING_APPROVAL" ||
      st.pendingVerification ||
      st.latestPayment?.status === "PENDING_VERIFICATION"
  ).length;
  const approvedCount = students.filter(
    (st: any) => st.userId?.status === "ACTIVE" && (st.hasPaid || st.latestPayment?.status === "PAID")
  ).length;
  const trialExpiredCount = students.filter((st: any) => st.trial?.isTrialExpired && !st.hasPaid).length;
  ...
  const filtered = students.filter((st: any) => {
    const matchesSearch =
      ...
      st.latestPayment?.transactionId?.toLowerCase().includes(search.toLowerCase());
    ...
    const matchesStatus =
      ...
      (selectedStatus === "ACTIVE" &&
        st.userId?.status === "ACTIVE" &&
        (st.hasPaid || st.latestPayment?.status === "PAID")) ||
      (selectedStatus === "TRIAL_EXPIRED" && st.trial?.isTrialExpired && !st.hasPaid) ||
      (selectedStatus === "SUSPENDED" && st.userId?.status === "SUSPENDED");
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-4 border-b border-slate-200 dark:border-slate-800">
        <div className="space-y-1">
          ...
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">
            Review student registrations, verify tuition fee payments, approve portal access, and manage records.
          </p>
        </div>

        {/* Status Filter Tabs with Counts (Including Payment Approvals) */}
        <div className="flex items-center gap-1.5 p-1 bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/60 rounded-xl overflow-x-auto self-start sm:self-auto shrink-0">
          {[
            ...
            { id: "ACTIVE", label: `Approved & Paid (${approvedCount})` },
            { id: "TRIAL_EXPIRED", label: `Trial Expired (${trialExpiredCount})` },
                ...
                >
                  ...
                  <div className="col-span-3 space-y-0.5">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {st.pendingVerification || st.latestPayment?.status === "PENDING_VERIFICATION" ? (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold bg-amber-50 text-amber-800 dark:bg-amber-950 dark:text-amber-300 border border-amber-300 dark:border-amber-700 animate-pulse">
                          <span>Payment Submitted: ₹{st.latestPayment?.amount || 1999}</span>
                          {st.latestPayment?.transactionId && (
                            <span className="font-mono font-normal">({st.latestPayment.transactionId})</span>
                          )}
                        </span>
                      ) : st.hasPaid || st.latestPayment?.status === "PAID" ? (
                      ...
                      ) : st.trial?.isTrialActive ? (
                        <span className="inline-flex items-center px-1.5 py-0.2 rounded text-[10px] font-bold bg-blue-50 text-[#004b79] dark:bg-[#002137] dark:text-[#dfb74a] border border-blue-200 dark:border-[#004b79]/60">
                          Trial Active ({st.trial?.remainingHours || 48}h)
                        </span>
                  ...
                  <div className="col-span-2 flex items-center justify-start md:justify-end gap-1.5">
                    {st.pendingVerification || st.latestPayment?.status === "PENDING_VERIFICATION" ? (
                      <button
                        type="button"
                        onClick={() => handleQuickApprove(st, true)}
                        className="px-2.5 py-1 rounded-lg text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white transition-colors cursor-pointer flex items-center gap-1 shadow-sm"
                        title="Approve student payment & unlock portal access"
                      >
